...
const AdminPage = async () => {
  if (isAuthenticated() && getAuthenticatedUser().isAnAdmin === true) {
    ...
    const response = await fetch(`${process.env.API_BASE_URL}/admin`, options);
...
async function getVerifiedList() {
  ...
  const response2 = await fetch(`${process.env.API_BASE_URL}/admin/all`, options);
...
function renderQuizList(quizList) {
  ...
  quizList.forEach((element) => {
    ...
    btnRemove.addEventListener('click', async (e) => {
      ...
      const response = await fetch(`${process.env.API_BASE_URL}/admin/remove/${pointer}`, options);
      if (!response.ok)
    ...
    quizElementTitle.addEventListener('click', async (e) => {
      ...
      const response = await fetch(`${process.env.API_BASE_URL}/admin/quiz/${pointer}`, options);
      if (!response.ok)
        throw new Error(`fetch error : ${response.status} : ${response.statusText}`);
...
async function validateQuiz(quiz) {
  ...
  const response = await fetch(`${process.env.API_BASE_URL}/admin/validate/${quiz}`, options);
  if (!response.ok) throw new Error(`fetch error : ${response.status} : ${response.statusText}`);
  Navigate('/admin');
}
